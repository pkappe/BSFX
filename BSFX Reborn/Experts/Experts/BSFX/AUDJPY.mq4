...
//|                for the Australian Dollar/Japanese Yen            |
//+------------------------------------------------------------------+
   ...
   if(Symbol()=="AUDJPY" && justTraded==false)
     {
      ...
      if(audjpyMove>=Move && spread<=MaxSpread && justTraded==false)
        {
         double stopLoss=audjpyBid-StopLoss;
         double goal=Goal+audjpyBid;
         ticket=OrderSend("AUDJPY",OP_BUY,lotSize,audjpyBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(audjpyMove<=(Move*-1) && spread<MaxSpread && justTraded==false)
        {
         double shortStop=audjpyAsk+StopLoss;
         double shortGoal=Goal-audjpyAsk;
         ticket=OrderSend("AUDJPY",OP_SELL,lotSize,audjpyAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
