   ...
   if(Symbol()=="EURJPY" && justTraded==false)
     {
      ...
      if(eurjpyMove>=Move && spread<=MaxSpread && justTraded==false)
        {
         double stopLoss=eurjpyBid-StopLoss;
         double goal=Goal+eurjpyBid;
         ticket=OrderSend("EURJPY",OP_BUY,lotSize,eurjpyBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(eurjpyMove<=(Move*-1) && spread<MaxSpread && justTraded==false)
        {
         double shortStop=eurjpyAsk+StopLoss;
         double shortGoal=Goal-eurjpyAsk;
         ticket=OrderSend("EURJPY",OP_SELL,lotSize,eurjpyAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
