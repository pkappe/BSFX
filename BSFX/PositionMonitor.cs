			...
			try
			{
				...
				if (offerTableRow.Instrument == "AUD/CAD")
				{
					...
					if (audcadThirty.Text != "--")
					{
						...
						if (audcadSideString == "Long")
						{
							// Subtract entry price from bid price
							audcadPosMoveDec = Decimal.Subtract(bidd, audcadEntryPrice);
							// P/L or Position move string conversion
							string moveString = Convert.ToString(audcadPosMoveDec);
							...
							// Changes to P/L
							if (audcadPosMoveDec >= 0)
							{
								actiBox.Text = audcadPosMoveString;
								audcadPL.BackColor = System.Drawing.Color.Blue;
								this.Invoke(new MethodInvoker(delegate { audcadPL.Text = "$" + profitOne + "." + profitTwo; }));
							}
							else if (audcadPosMoveDec < 0)
							{
								actiBox.Text = audcadPosMoveString;
							...
							// LONG: Sell Trigger
							if (audcadPosMoveDec > goalLong)
							{
								actiBox.Text = audcadPosMoveString;
								netPandL = netPandL + Convert.ToDecimal(startPL);
								audcadEntryPrice = 0;
								audcadSideString = "";
								this.Invoke(new MethodInvoker(delegate { actiBox.AppendText(DateTime.Now + ": CLOSED:" + offerTableRow.Instrument + " @ " + offerTableRow.Bid + " For:" + audcadPL.Text + Environment.NewLine); }));
						...
						else if (audcadSideString == "Short")
						{
							// Subtract entry price from bid price
							audcadPosMoveShortDec = (askd - audcadEntryPrice);
							// P/L or Position move string conversion
