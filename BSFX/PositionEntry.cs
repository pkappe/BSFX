					...
					if (offerTableRow.Instrument == "AUD/CAD")
					{
						...
						if (audcadThirty.Text != "--")
						{
							...
							if (move.StartsWith("0.00") && moveDouble >= 0)
							{
								...
								audcadMove.BackColor = System.Drawing.Color.Blue;
								audcadMoveInt = moveDouble;
								string moveTwo = move.Substring(4, 2);
								this.Invoke(new MethodInvoker(delegate { audcadMove.Text = moveTwo; }));

								// Check if a Buy Long is triggered
								if (buyCheck >= buyLong & startSpread < maxSpread && audcadEntry.Text == "--")
								{
									actiBox.Text = audcadPosMoveString;
									audcadSideString = "Long";
									audcadEntryPrice = Convert.ToDecimal(ask);
									this.Invoke(new MethodInvoker(delegate { audcadEntry.Text = asks; }));
									this.Invoke(new MethodInvoker(delegate { audcadSide.Text = "Long"; }));
							...
							if (move.StartsWith("-0.00") && moveDouble < 0)
							{
								...
								audcadMove.BackColor = System.Drawing.Color.Red;
								this.Invoke(new MethodInvoker(delegate { audcadMove.Text = "(" + moveTwo + ")"; }));

								// Check if a Sell Short is triggered
								if (buyCheck <= sellShort & startSpread < maxSpread && audcadEntry.Text == "--")
								{
									actiBox.Text = audcadPosMoveString;
									...
									audcadEntryPrice = Convert.ToDecimal(bid);
									this.Invoke(new MethodInvoker(delegate { audcadEntry.Text = bids; }));
									audcadSideString = "Short";
									this.Invoke(new MethodInvoker(delegate { audcadSide.Text = "Short"; }));
								}
							}
						}
					}
