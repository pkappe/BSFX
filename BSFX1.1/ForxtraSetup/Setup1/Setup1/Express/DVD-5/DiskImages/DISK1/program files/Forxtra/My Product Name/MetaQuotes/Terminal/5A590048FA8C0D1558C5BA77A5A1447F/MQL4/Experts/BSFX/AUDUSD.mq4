...
double audusdMove;
string audusdSide;
double audusdPastPrice;
double audusdPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   audusdPastPriceCheck=double_array[0][1];
   if(audusdPastPriceCheck==audusdPastPrice && justTraded==true)
     {
      audusdPastPrice=audusdPastPrice;
      justTraded=true;
     }
   else
     {
      audusdPastPrice=audusdPastPriceCheck;
      justTraded=false;
     }
// Set Bid/Ask and Spread
   audusdBid = MarketInfo("AUDUSD",MODE_BID);
   audusdAsk = MarketInfo("AUDUSD",MODE_ASK);
   spread=audusdAsk-audusdBid;

// Calculate the move
   audusdMove=audusdAsk-audusdPastPrice;
   string textMove=audusdMove*10000;
   ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="AUDUSD" && justTraded==false)
     {

      // LONG POSITION ENTRY CHECK
      if(audusdMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=audusdBid-StopLoss;
         double goal=Goal+audusdBid;
         justTraded = true;
         ticket=OrderSend("AUDUSD",OP_BUY,lotSize,audusdBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(audusdMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+audusdAsk;
         double shortGoal=audusdAsk-Goal;
         ticket=OrderSend("AUDUSD",OP_SELL,lotSize,audusdAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
