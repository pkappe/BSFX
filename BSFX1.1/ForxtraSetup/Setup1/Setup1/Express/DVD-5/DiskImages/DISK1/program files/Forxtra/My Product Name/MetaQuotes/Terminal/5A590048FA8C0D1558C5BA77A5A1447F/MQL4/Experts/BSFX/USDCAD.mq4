...
double usdcadMove;
string usdcadSide;
double usdcadPastPrice;
double usdcadPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   usdcadPastPriceCheck=double_array[0][1];
   if(usdcadPastPriceCheck==usdcadPastPrice && justTraded==true)
     {
      usdcadPastPrice=usdcadPastPrice;
      justTraded=true;
     }
   else
     {
      usdcadPastPrice=usdcadPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      usdcadBid = MarketInfo("USDCAD",MODE_BID);
      usdcadAsk = MarketInfo("USDCAD",MODE_ASK);
      spread=usdcadAsk-usdcadBid;

      // Calculate the move
      usdcadMove=usdcadAsk-usdcadPastPrice;
      string textMove = usdcadMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="USDCAD" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(usdcadMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=usdcadBid-StopLoss;
         double goal=Goal+usdcadBid;
         justTraded = true;
         ticket=OrderSend("USDCAD",OP_BUY,lotSize,usdcadBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(usdcadMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+usdcadAsk;
         double shortGoal=usdcadAsk-Goal;
         ticket=OrderSend("USDCAD",OP_SELL,lotSize,usdcadAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
