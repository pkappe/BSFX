...
double eurnzdMove;
string eurnzdSide;
double eurnzdPastPrice;
double eurnzdPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   eurnzdPastPriceCheck=double_array[0][1];
   if(eurnzdPastPriceCheck==eurnzdPastPrice && justTraded==true)
     {
      eurnzdPastPrice=eurnzdPastPrice;
      justTraded=true;
     }
   else
     {
      eurnzdPastPrice=eurnzdPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      eurnzdBid = MarketInfo("EURNZD",MODE_BID);
      eurnzdAsk = MarketInfo("EURNZD",MODE_ASK);
      spread=eurnzdAsk-eurnzdBid;

      // Calculate the move
      eurnzdMove=eurnzdAsk-eurnzdPastPrice;
      string textMove = eurnzdMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="EURNZD" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(eurnzdMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=eurnzdBid-StopLoss;
         double goal=Goal+eurnzdBid;
         justTraded = true;
         ticket=OrderSend("EURNZD",OP_BUY,lotSize,eurnzdBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(eurnzdMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+eurnzdAsk;
         double shortGoal=eurnzdAsk-Goal;
         ticket=OrderSend("EURNZD",OP_SELL,lotSize,eurnzdAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
