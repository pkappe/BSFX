...
//|             for the New Zealand Dollar/Japanese Yen              |
//+------------------------------------------------------------------+
   ...
   if(Symbol()=="NZDJPY" && justTraded==false)
     {
      ...
      if(nzdjpyMove>=Move && spread<=MaxSpread && justTraded==false)
        {
         double stopLoss=nzdjpyBid-StopLoss;
         double goal=Goal+nzdjpyBid;
         ticket=OrderSend("NZDJPY",OP_BUY,lotSize,nzdjpyBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(nzdjpyMove<=(Move*-1) && spread<MaxSpread && justTraded==false)
        {
         double shortStop=nzdjpyAsk+StopLoss;
         double shortGoal=Goal-nzdjpyAsk;
         ticket=OrderSend("NZDJPY",OP_SELL,lotSize,nzdjpyAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
