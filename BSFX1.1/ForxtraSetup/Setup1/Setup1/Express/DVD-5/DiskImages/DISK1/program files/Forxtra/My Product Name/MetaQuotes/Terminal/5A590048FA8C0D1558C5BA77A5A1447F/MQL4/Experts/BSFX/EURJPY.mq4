...
double eurjpyMove;
string eurjpySide;
double eurjpyPastPrice;
double eurjpyPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   eurjpyPastPriceCheck=double_array[0][1]/100;
   if(eurjpyPastPriceCheck==eurjpyPastPrice && justTraded==true)
     {
      eurjpyPastPrice=eurjpyPastPrice;
      justTraded=true;
     }
   else
     {
      eurjpyPastPrice=eurjpyPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      eurjpyBid = MarketInfo("EURJPY",MODE_BID)/100;
      eurjpyAsk = MarketInfo("EURJPY",MODE_ASK)/100;
      spread=eurjpyAsk-eurjpyBid;

      // Calculate the move
      eurjpyMove=eurjpyAsk-eurjpyPastPrice;
      string textMove=eurjpyMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="EURJPY" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(eurjpyMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=(eurjpyBid-StopLoss)*100;
         double goal=(Goal+eurjpyBid)*100;
         justTraded = true;
         ticket=OrderSend("EURJPY",OP_BUY,lotSize,(eurjpyBid*100),3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(eurjpyMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=(StopLoss+eurjpyAsk)*100;
         double shortGoal=(eurjpyAsk-Goal)*100;
         ticket=OrderSend("EURJPY",OP_SELL,lotSize,(eurjpyAsk*100),3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
