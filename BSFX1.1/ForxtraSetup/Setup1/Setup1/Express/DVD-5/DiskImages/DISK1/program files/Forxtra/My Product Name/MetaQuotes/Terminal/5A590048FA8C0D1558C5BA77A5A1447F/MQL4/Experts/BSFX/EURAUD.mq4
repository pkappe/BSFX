...
double euraudMove;
string euraudSide;
double euraudPastPrice;
double euraudPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   euraudPastPriceCheck=double_array[0][1];
   if(euraudPastPriceCheck==euraudPastPrice && justTraded==true)
     {
      euraudPastPrice=euraudPastPrice;
      justTraded=true;
     }
   else
     {
      euraudPastPrice=euraudPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      euraudBid = MarketInfo("EURAUD",MODE_BID);
      euraudAsk = MarketInfo("EURAUD",MODE_ASK);
      spread=euraudAsk-euraudBid;

      // Calculate the move
      euraudMove=euraudAsk-euraudPastPrice;
      string textMove = euraudMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="EURAUD" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(euraudMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=euraudBid-StopLoss;
         double goal=Goal+euraudBid;
         justTraded = true;
         ticket=OrderSend("EURAUD",OP_BUY,lotSize,euraudBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(euraudMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+euraudAsk;
         double shortGoal=euraudAsk-Goal;
         ticket=OrderSend("EURAUD",OP_SELL,lotSize,euraudAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
