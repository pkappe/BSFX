...
double gbpaudMove;
string gbpaudSide;
double gbpaudPastPrice;
double gbpaudPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   gbpaudPastPriceCheck=double_array[0][1];
   if(gbpaudPastPriceCheck==gbpaudPastPrice && justTraded==true)
     {
      gbpaudPastPrice=gbpaudPastPrice;
      justTraded=true;
     }
   else
     {
      gbpaudPastPrice=gbpaudPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      gbpaudBid = MarketInfo("GBPAUD",MODE_BID);
      gbpaudAsk = MarketInfo("GBPAUD",MODE_ASK);
      spread=gbpaudAsk-gbpaudBid;

      // Calculate the move
      gbpaudMove=gbpaudAsk-gbpaudPastPrice;
      string textMove = gbpaudMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="GBPAUD" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(gbpaudMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=gbpaudBid-StopLoss;
         double goal=Goal+gbpaudBid;
         justTraded = true;
         ticket=OrderSend("GBPAUD",OP_BUY,lotSize,gbpaudBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(gbpaudMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+gbpaudAsk;
         double shortGoal=gbpaudAsk-Goal;
         ticket=OrderSend("GBPAUD",OP_SELL,lotSize,gbpaudAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
