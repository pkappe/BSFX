...
//|                for the Canadian Dollar/Japanese Yen              |
//+------------------------------------------------------------------+
   ...
   if(Symbol()=="CADJPY" && justTraded==false)
     {
      ...
      if(cadjpyMove>=Move && spread<=MaxSpread && justTraded==false)
        {
         double stopLoss=cadjpyBid-StopLoss;
         double goal=Goal+cadjpyBid;
         ticket=OrderSend("CADJPY",OP_BUY,lotSize,cadjpyBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(cadjpyMove<=(Move*-1) && spread<MaxSpread && justTraded==false)
        {
         double shortStop=cadjpyAsk+StopLoss;
         double shortGoal=Goal-cadjpyAsk;
         ticket=OrderSend("CADJPY",OP_SELL,lotSize,cadjpyAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
