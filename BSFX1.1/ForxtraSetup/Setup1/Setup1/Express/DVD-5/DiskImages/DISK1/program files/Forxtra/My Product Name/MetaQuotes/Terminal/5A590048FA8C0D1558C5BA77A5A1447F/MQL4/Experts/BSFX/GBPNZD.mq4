...
double gbpnzdMove;
string gbpnzdSide;
double gbpnzdPastPrice;
double gbpnzdPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   gbpnzdPastPriceCheck=double_array[0][1];
   if(gbpnzdPastPriceCheck==gbpnzdPastPrice && justTraded==true)
     {
      gbpnzdPastPrice=gbpnzdPastPrice;
      justTraded=true;
     }
   else
     {
      gbpnzdPastPrice=gbpnzdPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      gbpnzdBid = MarketInfo("GBPNZD",MODE_BID);
      gbpnzdAsk = MarketInfo("GBPNZD",MODE_ASK);
      spread=gbpnzdAsk-gbpnzdBid;

      // Calculate the move
      gbpnzdMove=gbpnzdAsk-gbpnzdPastPrice;
      string textMove = gbpnzdMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="GBPNZD" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(gbpnzdMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=gbpnzdBid-StopLoss;
         double goal=Goal+gbpnzdBid;
         justTraded = true;
         ticket=OrderSend("GBPNZD",OP_BUY,lotSize,gbpnzdBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(gbpnzdMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+gbpnzdAsk;
         double shortGoal=gbpnzdAsk-Goal;
         ticket=OrderSend("GBPNZD",OP_SELL,lotSize,gbpnzdAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
