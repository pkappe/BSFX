...
double eurusdMove;
string eurusdSide;
double eurusdPastPrice;
double eurusdPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   eurusdPastPriceCheck=double_array[0][1];
   if(eurusdPastPriceCheck==eurusdPastPrice && justTraded==true)
     {
      eurusdPastPrice=eurusdPastPrice;
      justTraded=true;
     }
   else
     {
      eurusdPastPrice=eurusdPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      eurusdBid = MarketInfo("EURUSD",MODE_BID);
      eurusdAsk = MarketInfo("EURUSD",MODE_ASK);
      spread=eurusdAsk-eurusdBid;

      // Calculate the move
      eurusdMove=eurusdAsk-eurusdPastPrice;
      string textMove = eurusdMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="EURUSD" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(eurusdMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=eurusdBid-StopLoss;
         double goal=Goal+eurusdBid;
         justTraded = true;
         ticket=OrderSend("EURUSD",OP_BUY,lotSize,eurusdBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(eurusdMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+eurusdAsk;
         double shortGoal=eurusdAsk-Goal;
         ticket=OrderSend("EURUSD",OP_SELL,lotSize,eurusdAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
