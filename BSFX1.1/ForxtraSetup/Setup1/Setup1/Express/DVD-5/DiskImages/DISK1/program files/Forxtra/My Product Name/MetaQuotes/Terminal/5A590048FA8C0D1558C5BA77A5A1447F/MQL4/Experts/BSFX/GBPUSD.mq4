...
double gbpusdMove;
string gbpusdSide;
double gbpusdPastPrice;
double gbpusdPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   gbpusdPastPriceCheck=double_array[0][1];
   if(gbpusdPastPriceCheck==gbpusdPastPrice && justTraded==true)
     {
      gbpusdPastPrice=gbpusdPastPrice;
      justTraded=true;
     }
   else
     {
      gbpusdPastPrice=gbpusdPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      gbpusdBid = MarketInfo("GBPUSD",MODE_BID);
      gbpusdAsk = MarketInfo("GBPUSD",MODE_ASK);
      spread=gbpusdAsk-gbpusdBid;

      // Calculate the move
      gbpusdMove=gbpusdAsk-gbpusdPastPrice;
      string textMove = gbpusdMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="GBPUSD" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(gbpusdMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=gbpusdBid-StopLoss;
         double goal=Goal+gbpusdBid;
         justTraded = true;
         ticket=OrderSend("GBPUSD",OP_BUY,lotSize,gbpusdBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(gbpusdMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+gbpusdAsk;
         double shortGoal=gbpusdAsk-Goal;
         ticket=OrderSend("GBPUSD",OP_SELL,lotSize,gbpusdAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
