...
double chfjpyMove;
string chfjpySide;
double chfjpyPastPrice;
double chfjpyPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   chfjpyPastPriceCheck=double_array[0][1]/100;
   if(chfjpyPastPriceCheck==chfjpyPastPrice && justTraded==true)
     {
      chfjpyPastPrice=chfjpyPastPrice;
      justTraded=true;
     }
   else
     {
      chfjpyPastPrice=chfjpyPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      chfjpyBid = MarketInfo("CHFJPY",MODE_BID)/100;
      chfjpyAsk = MarketInfo("CHFJPY",MODE_ASK)/100;
      spread=chfjpyAsk-chfjpyBid;

      // Calculate the move
      chfjpyMove=chfjpyAsk-chfjpyPastPrice;
      string textMove=chfjpyMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="CHFJPY" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(chfjpyMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=(chfjpyBid-StopLoss)*100;
         double goal=(Goal+chfjpyBid)*100;
         justTraded = true;
         ticket=OrderSend("CHFJPY",OP_BUY,lotSize,(chfjpyBid*100),3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(chfjpyMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=(StopLoss+chfjpyAsk)*100;
         double shortGoal=(chfjpyAsk-Goal)*100;
         ticket=OrderSend("CHFJPY",OP_SELL,lotSize,(chfjpyAsk*100),3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
