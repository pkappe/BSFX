...
double eurgbpMove;
string eurgbpSide;
double eurgbpPastPrice;
double eurgbpPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   eurgbpPastPriceCheck=double_array[0][1];
   if(eurgbpPastPriceCheck==eurgbpPastPrice && justTraded==true)
     {
      eurgbpPastPrice=eurgbpPastPrice;
      justTraded=true;
     }
   else
     {
      eurgbpPastPrice=eurgbpPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      eurgbpBid = MarketInfo("EURGBP",MODE_BID);
      eurgbpAsk = MarketInfo("EURGBP",MODE_ASK);
      spread=eurgbpAsk-eurgbpBid;

      // Calculate the move
      eurgbpMove=eurgbpAsk-eurgbpPastPrice;
      string textMove = eurgbpMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="EURGBP" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(eurgbpMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=eurgbpBid-StopLoss;
         double goal=Goal+eurgbpBid;
         justTraded = true;
         ticket=OrderSend("EURGBP",OP_BUY,lotSize,eurgbpBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(eurgbpMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+eurgbpAsk;
         double shortGoal=eurgbpAsk-Goal;
         ticket=OrderSend("EURGBP",OP_SELL,lotSize,eurgbpAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
