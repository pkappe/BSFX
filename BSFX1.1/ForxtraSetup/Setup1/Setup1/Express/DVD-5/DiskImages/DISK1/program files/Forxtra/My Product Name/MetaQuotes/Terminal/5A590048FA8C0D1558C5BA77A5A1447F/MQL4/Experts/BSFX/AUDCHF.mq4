...
double audchfMove;
string audchfSide;
double audchfPastPrice;
double audchfPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   audchfPastPriceCheck=double_array[0][1];
   if(audchfPastPriceCheck==audchfPastPrice && justTraded==true)
     {
      audchfPastPrice=audchfPastPrice;
      justTraded=true;
     }
   else
     {
      audchfPastPrice=audchfPastPriceCheck;
      justTraded=false;
     }
// Set Bid/Ask and Spread
   audchfBid = MarketInfo("AUDCHF",MODE_BID);
   audchfAsk = MarketInfo("AUDCHF",MODE_ASK);
   spread=audchfAsk-audchfBid;

// Calculate the move
   audchfMove=audchfAsk-audchfPastPrice;
   string textMove=audchfMove*10000;
   ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="AUDCHF" && justTraded==false)
     {

      // LONG POSITION ENTRY CHECK
      if(audchfMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=audchfBid-StopLoss;
         double goal=Goal+audchfBid;
         justTraded = true;
         ticket=OrderSend("AUDCHF",OP_BUY,lotSize,audchfBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(audchfMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+audchfAsk;
         double shortGoal=audchfAsk-Goal;
         ticket=OrderSend("AUDCHF",OP_SELL,lotSize,audchfAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
