...
double cadjpyMove;
string cadjpySide;
double cadjpyPastPrice;
double cadjpyPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   cadjpyPastPriceCheck=double_array[0][1]/100;
   if(cadjpyPastPriceCheck==cadjpyPastPrice && justTraded==true)
     {
      cadjpyPastPrice=cadjpyPastPrice;
      justTraded=true;
     }
   else
     {
      cadjpyPastPrice=cadjpyPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      cadjpyBid = MarketInfo("CADJPY",MODE_BID)/100;
      cadjpyAsk = MarketInfo("CADJPY",MODE_ASK)/100;
      spread=cadjpyAsk-cadjpyBid;

      // Calculate the move
      cadjpyMove=cadjpyAsk-cadjpyPastPrice;
      string textMove=cadjpyMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="CADJPY" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(cadjpyMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=(cadjpyBid-StopLoss)*100;
         double goal=(Goal+cadjpyBid)*100;
         justTraded = true;
         ticket=OrderSend("CADJPY",OP_BUY,lotSize,(cadjpyBid*100),3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(cadjpyMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=(StopLoss+cadjpyAsk)*100;
         double shortGoal=(cadjpyAsk-Goal)*100;
         ticket=OrderSend("CADJPY",OP_SELL,lotSize,(cadjpyAsk*100),3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
