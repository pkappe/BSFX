...
double usdchfMove;
string usdchfSide;
double usdchfPastPrice;
double usdchfPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   usdchfPastPriceCheck=double_array[0][1];
   if(usdchfPastPriceCheck==usdchfPastPrice && justTraded==true)
     {
      usdchfPastPrice=usdchfPastPrice;
      justTraded=true;
     }
   else
     {
      usdchfPastPrice=usdchfPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      usdchfBid = MarketInfo("USDCHF",MODE_BID);
      usdchfAsk = MarketInfo("USDCHF",MODE_ASK);
      spread=usdchfAsk-usdchfBid;

      // Calculate the move
      usdchfMove=usdchfAsk-usdchfPastPrice;
      string textMove = usdchfMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="USDCHF" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(usdchfMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=usdchfBid-StopLoss;
         double goal=Goal+usdchfBid;
         justTraded = true;
         ticket=OrderSend("USDCHF",OP_BUY,lotSize,usdchfBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(usdchfMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+usdchfAsk;
         double shortGoal=usdchfAsk-Goal;
         ticket=OrderSend("USDCHF",OP_SELL,lotSize,usdchfAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
