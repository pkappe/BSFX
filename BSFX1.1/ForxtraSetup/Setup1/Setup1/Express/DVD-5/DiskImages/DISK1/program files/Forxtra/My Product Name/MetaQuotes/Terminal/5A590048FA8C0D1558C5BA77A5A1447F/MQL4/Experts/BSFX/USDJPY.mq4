...
double usdjpyMove;
string usdjpySide;
double usdjpyPastPrice;
double usdjpyPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   usdjpyPastPriceCheck=double_array[0][1]/100;
   if(usdjpyPastPriceCheck==usdjpyPastPrice && justTraded==true)
     {
      usdjpyPastPrice=usdjpyPastPrice;
      justTraded=true;
     }
   else
     {
      usdjpyPastPrice=usdjpyPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      usdjpyBid = MarketInfo("USDJPY",MODE_BID)/100;
      usdjpyAsk = MarketInfo("USDJPY",MODE_ASK)/100;
      spread=usdjpyAsk-usdjpyBid;

      // Calculate the move
      usdjpyMove=usdjpyAsk-usdjpyPastPrice;
      string textMove=usdjpyMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="USDJPY" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(usdjpyMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=(usdjpyBid-StopLoss)*100;
         double goal=(Goal+usdjpyBid)*100;
         justTraded = true;
         ticket=OrderSend("USDJPY",OP_BUY,lotSize,(usdjpyBid*100),3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(usdjpyMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=(StopLoss+usdjpyAsk)*100;
         double shortGoal=(usdjpyAsk-Goal)*100;
         ticket=OrderSend("USDJPY",OP_SELL,lotSize,(usdjpyAsk*100),3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
