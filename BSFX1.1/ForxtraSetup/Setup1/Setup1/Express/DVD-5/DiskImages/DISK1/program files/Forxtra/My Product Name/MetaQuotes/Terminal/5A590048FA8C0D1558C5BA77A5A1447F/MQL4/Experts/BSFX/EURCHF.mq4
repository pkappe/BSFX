...
double eurchfMove;
string eurchfSide;
double eurchfPastPrice;
double eurchfPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   eurchfPastPriceCheck=double_array[0][1];
   if(eurchfPastPriceCheck==eurchfPastPrice && justTraded==true)
     {
      eurchfPastPrice=eurchfPastPrice;
      justTraded=true;
     }
   else
     {
      eurchfPastPrice=eurchfPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      eurchfBid = MarketInfo("EURCHF",MODE_BID);
      eurchfAsk = MarketInfo("EURCHF",MODE_ASK);
      spread=eurchfAsk-eurchfBid;

      // Calculate the move
      eurchfMove=eurchfAsk-eurchfPastPrice;
      string textMove = eurchfMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="EURCHF" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(eurchfMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=eurchfBid-StopLoss;
         double goal=Goal+eurchfBid;
         justTraded = true;
         ticket=OrderSend("EURCHF",OP_BUY,lotSize,eurchfBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(eurchfMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+eurchfAsk;
         double shortGoal=eurchfAsk-Goal;
         ticket=OrderSend("EURCHF",OP_SELL,lotSize,eurchfAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
