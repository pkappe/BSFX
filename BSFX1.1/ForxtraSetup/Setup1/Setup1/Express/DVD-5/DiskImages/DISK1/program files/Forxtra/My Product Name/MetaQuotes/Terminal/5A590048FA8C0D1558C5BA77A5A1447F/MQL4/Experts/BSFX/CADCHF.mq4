...
double cadchfMove;
string cadchfSide;
double cadchfPastPrice;
double cadchfPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   cadchfPastPriceCheck=double_array[0][1];
   if(cadchfPastPriceCheck==cadchfPastPrice && justTraded==true)
     {
      cadchfPastPrice=cadchfPastPrice;
      justTraded=true;
     }
   else
     {
      cadchfPastPrice=cadchfPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      cadchfBid = MarketInfo("CADCHF",MODE_BID);
      cadchfAsk = MarketInfo("CADCHF",MODE_ASK);
      spread=cadchfAsk-cadchfBid;

      // Calculate the move
      cadchfMove=cadchfAsk-cadchfPastPrice;
      string textMove = cadchfMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="CADCHF" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(cadchfMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=cadchfBid-StopLoss;
         double goal=Goal+cadchfBid;
         justTraded = true;
         ticket=OrderSend("CADCHF",OP_BUY,lotSize,cadchfBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(cadchfMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+cadchfAsk;
         double shortGoal=cadchfAsk-Goal;
         ticket=OrderSend("CADCHF",OP_SELL,lotSize,cadchfAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
