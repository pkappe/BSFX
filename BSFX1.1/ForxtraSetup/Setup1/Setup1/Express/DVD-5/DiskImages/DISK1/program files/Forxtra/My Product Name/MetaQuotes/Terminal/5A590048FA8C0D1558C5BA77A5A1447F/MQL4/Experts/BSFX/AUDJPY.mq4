...
double audjpyMove;
string audjpySide;
double audjpyPastPrice;
double audjpyPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   audjpyPastPriceCheck=double_array[0][1]/100;
   if(audjpyPastPriceCheck==audjpyPastPrice && justTraded==true)
     {
      audjpyPastPrice=audjpyPastPrice;
      justTraded=true;
     }
   else
     {
      audjpyPastPrice=audjpyPastPriceCheck;
      justTraded=false;
     }
// Set Bid/Ask and Spread
   audjpyBid = MarketInfo("AUDJPY",MODE_BID)/100;
   audjpyAsk = MarketInfo("AUDJPY",MODE_ASK)/100;
   spread=audjpyAsk-audjpyBid;

// Calculate the move
   audjpyMove=audjpyAsk-audjpyPastPrice;
   string textMove=audjpyMove*10000;
   ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="AUDJPY" && justTraded==false)
     {

      // LONG POSITION ENTRY CHECK
      if(audjpyMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=(audjpyBid-StopLoss)*100;
         double goal=(Goal+audjpyBid)*100;
         justTraded = true;
         ticket=OrderSend("AUDJPY",OP_BUY,lotSize,(audjpyBid*100),3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(audjpyMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=(StopLoss+audjpyAsk)*100;
         double shortGoal=(audjpyAsk-Goal)*100;
         ticket=OrderSend("AUDJPY",OP_SELL,lotSize,(audjpyAsk*100),3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
