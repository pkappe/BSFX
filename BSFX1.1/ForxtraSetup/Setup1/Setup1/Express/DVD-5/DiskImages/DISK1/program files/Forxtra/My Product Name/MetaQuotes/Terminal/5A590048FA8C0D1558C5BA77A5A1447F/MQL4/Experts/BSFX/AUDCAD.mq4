...
double audcadMove;
string audcadSide;
double audcadPastPrice;
double audcadPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   audcadPastPriceCheck=double_array[0][1];
   if(audcadPastPriceCheck==audcadPastPrice && justTraded==true)
     {
      audcadPastPrice=audcadPastPrice;
      justTraded=true;
     }
   else
     {
      audcadPastPrice=audcadPastPriceCheck;
      justTraded=false;
     }
     
     // Set Bid/Ask and Spread
      audcadBid = MarketInfo("AUDCAD",MODE_BID);
      audcadAsk = MarketInfo("AUDCAD",MODE_ASK);
      spread=audcadAsk-audcadBid;

      // Calculate the move
      audcadMove=audcadAsk-audcadPastPrice;
      string textMove=audcadMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="AUDCAD" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(audcadMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=audcadBid-StopLoss;
         double goal=Goal+audcadBid;
         justTraded = true;
         ticket=OrderSend("AUDCAD",OP_BUY,lotSize,audcadBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(audcadMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+audcadAsk;
         double shortGoal=audcadAsk-Goal;
         ticket=OrderSend("AUDCAD",OP_SELL,lotSize,audcadAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
