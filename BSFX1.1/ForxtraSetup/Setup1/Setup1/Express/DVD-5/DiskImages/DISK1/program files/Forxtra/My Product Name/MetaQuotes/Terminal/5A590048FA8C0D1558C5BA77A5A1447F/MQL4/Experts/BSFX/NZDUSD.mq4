...
double nzdusdMove;
string nzdusdSide;
double nzdusdPastPrice;
double nzdusdPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   nzdusdPastPriceCheck=double_array[0][1];
   if(nzdusdPastPriceCheck==nzdusdPastPrice && justTraded==true)
     {
      nzdusdPastPrice=nzdusdPastPrice;
      justTraded=true;
     }
   else
     {
      nzdusdPastPrice=nzdusdPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      nzdusdBid = MarketInfo("NZDUSD",MODE_BID);
      nzdusdAsk = MarketInfo("NZDUSD",MODE_ASK);
      spread=nzdusdAsk-nzdusdBid;

      // Calculate the move
      nzdusdMove=nzdusdAsk-nzdusdPastPrice;
      string textMove = nzdusdMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="NZDUSD" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(nzdusdMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=nzdusdBid-StopLoss;
         double goal=Goal+nzdusdBid;
         justTraded = true;
         ticket=OrderSend("NZDUSD",OP_BUY,lotSize,nzdusdBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(nzdusdMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+nzdusdAsk;
         double shortGoal=nzdusdAsk-Goal;
         ticket=OrderSend("NZDUSD",OP_SELL,lotSize,nzdusdAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
