...
double eurcadMove;
string eurcadSide;
double eurcadPastPrice;
double eurcadPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   eurcadPastPriceCheck=double_array[0][1];
   if(eurcadPastPriceCheck==eurcadPastPrice && justTraded==true)
     {
      eurcadPastPrice=eurcadPastPrice;
      justTraded=true;
     }
   else
     {
      eurcadPastPrice=eurcadPastPriceCheck;
      justTraded=false;
     }
     // Set Bid/Ask and Spread
      eurcadBid = MarketInfo("EURCAD",MODE_BID);
      eurcadAsk = MarketInfo("EURCAD",MODE_ASK);
      spread=eurcadAsk-eurcadBid;

      // Calculate the move
      eurcadMove=eurcadAsk-eurcadPastPrice;
      string textMove = eurcadMove*10000;
      ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="EURCAD" && justTraded==false)
     {
      
      // LONG POSITION ENTRY CHECK
      if(eurcadMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=eurcadBid-StopLoss;
         double goal=Goal+eurcadBid;
         justTraded = true;
         ticket=OrderSend("EURCAD",OP_BUY,lotSize,eurcadBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(eurcadMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+eurcadAsk;
         double shortGoal=eurcadAsk-Goal;
         ticket=OrderSend("EURCAD",OP_SELL,lotSize,eurcadAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
