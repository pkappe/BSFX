...
// Grab the open price from the current candle
   ...
   gbpjpyPastPriceCheck=double_array[0][1]/100;
   if(gbpjpyPastPriceCheck==gbpjpyPastPrice && justTraded==true)
     {
      gbpjpyPastPrice=gbpjpyPastPrice;
      justTraded=true;
     }
   else
     {
      gbpjpyPastPrice=gbpjpyPastPriceCheck;
      justTraded=false;
     }
   ...
   if(Symbol()=="GBPJPY" && justTraded==false)
     {
      ...
      if(gbpjpyMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=(gbpjpyBid-StopLoss)*100;
         double goal=(Goal+gbpjpyBid)*100;
         justTraded = true;
         ticket=OrderSend("GBPJPY",OP_BUY,lotSize,(gbpjpyBid*100),3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(gbpjpyMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=(StopLoss+gbpjpyAsk)*100;
         double shortGoal=(gbpjpyAsk-Goal)*100;
         ticket=OrderSend("GBPJPY",OP_SELL,lotSize,(gbpjpyAsk*100),3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
