...
double audnzdMove;
string audnzdSide;
double audnzdPastPrice;
double audnzdPastPriceCheck;
...
// Grab the open price from the current candle
   ...
   audnzdPastPriceCheck=double_array[0][1];
   if(audnzdPastPriceCheck==audnzdPastPrice && justTraded==true)
     {
      audnzdPastPrice=audnzdPastPrice;
      justTraded=true;
     }
   else
     {
      audnzdPastPrice=audnzdPastPriceCheck;
      justTraded=false;
     }
// Set Bid/Ask and Spread
   audnzdBid = MarketInfo("AUDNZD",MODE_BID);
   audnzdAsk = MarketInfo("AUDNZD",MODE_ASK);
   spread=audnzdAsk-audnzdBid;

// Calculate the move
   audnzdMove=audnzdAsk-audnzdPastPrice;
   string textMove=audnzdMove*10000;
   ObjectSetString(0,"Label",OBJPROP_TEXT,textMove);
// See if the symbol is correct
   if(Symbol()=="AUDNZD" && justTraded==false)
     {

      // LONG POSITION ENTRY CHECK
      if(audnzdMove>=Move && spread<=MaxSpread)
        {
         double stopLoss=audnzdBid-StopLoss;
         double goal=Goal+audnzdBid;
         justTraded = true;
         ticket=OrderSend("AUDNZD",OP_BUY,lotSize,audnzdBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(audnzdMove<=(Move*-1) && spread<=MaxSpread)
        {
         justTraded=true;
         double shortStop=StopLoss+audnzdAsk;
         double shortGoal=audnzdAsk-Goal;
         ticket=OrderSend("AUDNZD",OP_SELL,lotSize,audnzdAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
