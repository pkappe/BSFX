...
//|            for the Great Britain Pound/Canadian Dollar           |
//+------------------------------------------------------------------+
   ...
   if(Symbol()=="GBPCAD" && justTraded==false)
     {
      ...
      if(gbpcadMove>=Move && spread<=MaxSpread && justTraded==false)
        {
      ...
      if(gbpcadMove<=(Move*-1) && spread<MaxSpread && justTraded==false)
        {
         double shortStop=gbpcadAsk+StopLoss;
         double shortGoal=Goal-gbpcadAsk;
