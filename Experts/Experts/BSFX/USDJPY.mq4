...
//|             for the U.S. Dollar/Japanese Yen                     |
//+------------------------------------------------------------------+
   ...
   if(Symbol()=="USDJPY" && justTraded==false)
     {
      ...
      if(usdjpyMove>=Move && spread<=MaxSpread && justTraded==false)
        {
         double stopLoss=usdjpyBid-StopLoss;
         double goal=Goal+usdjpyBid;
         ticket=OrderSend("USDJPY",OP_BUY,lotSize,usdjpyBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(usdjpyMove<=(Move*-1) && spread<MaxSpread && justTraded==false)
        {
         double shortStop=usdjpyAsk+StopLoss;
         double shortGoal=Goal-usdjpyAsk;
         ticket=OrderSend("USDJPY",OP_SELL,lotSize,usdjpyAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
