...
//|                 for the Swiss Franc/Japanese Yen                 |
//+------------------------------------------------------------------+
   ...
   if(Symbol()=="CHFJPY" && justTraded==false)
     {
      ...
      if(chfjpyMove>=Move && spread<=MaxSpread && justTraded==false)
        {
         double stopLoss=chfjpyBid-StopLoss;
         double goal=Goal+chfjpyBid;
         ticket=OrderSend("CHFJPY",OP_BUY,lotSize,chfjpyBid,3,stopLoss,goal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
      ...
      if(chfjpyMove<=(Move*-1) && spread<MaxSpread && justTraded==false)
        {
         double shortStop=chfjpyAsk+StopLoss;
         double shortGoal=Goal-chfjpyAsk;
         ticket=OrderSend("CHFJPY",OP_SELL,lotSize,chfjpyAsk,3,shortStop,shortGoal,NULL,0,0,clrGreen);
         if(ticket<0)
           {
            Print("OrderSend failed with error #",GetLastError());
           }
